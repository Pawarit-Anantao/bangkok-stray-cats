...
export default function SightingHistoryPage() {
    ...
    async function fetchSightings() {
      try {
        ...
        const formattedData = data.map((s: any) => ({
          ...
          health_tags: s.sighting_health_tags
            .map((st: any) => tagLabels?.find((t) => t.key === st.tag_key))
            .filter(Boolean),
        }));
  ...
  if (loading && currentPage === 1)
    return <div style={loadingStyle}>กำลังโหลดประวัติ...</div>;
          ...
          sightings.map((s) => (
            <div key={s.id} style={sightingCardStyle}>
              ...
              <div style={tagRowStyle}>
                {s.health_tags.length > 0 ? (
                  s.health_tags.map((tag: any) => (
                    <TagChip
                      key={tag.key}
                      label={tag.label_th}
                      category="health"
              ...
              <div style={noteSectionStyle}>
                <p style={noteTextStyle}>
                  {s.health_note || s.note || "ไม่มีรายละเอียดเพิ่มเติม"}
                </p>
              </div>
              ...
              <div style={locationBoxStyle}>
                ...
                <p style={locationTextStyle}>
                  {s.address_name ? `${s.address_name}: ` : ""}
                  {s.address_detail || "ไม่ระบุพิกัดที่ชัดเจน"}
                </p>
              </div>
            </div>
          ))
        ) : (
          <div style={noDataFullStyle}>
            ยังไม่มีประวัติการพบเห็นน้องแมวตัวนี้
          </div>
        )}
      </section>
...
const sightingCardStyle: React.CSSProperties = {
  background: "#FFF",
  borderRadius: "16px",
  padding: "16px",
...
const authorSubtextStyle: React.CSSProperties = {
  fontSize: "11px",
  fontWeight: 400,
  color: "#8F8362",
};
...
const noDataFullStyle: React.CSSProperties = {
  textAlign: "center",
  padding: "50px",
  color: "#8F8362",
  fontSize: "14px",
