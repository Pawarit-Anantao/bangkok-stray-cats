...
const mainWrapper: React.CSSProperties = {
  display: 'flex', flexDirection: 'column', gap: '6px', width: '336px', margin: '0 auto'
};

const barContainerStyle: React.CSSProperties = {
  display: 'flex', width: '100%', height: '42px', alignItems: 'center', position: 'relative', 
  borderRadius: '32px', border: '1px solid #8F8362', background: '#FFF', overflow: 'hidden'
};

const iconLeftStyle: React.CSSProperties = { paddingLeft: '15px', display: 'flex', alignItems: 'center' };
const inputStyle: React.CSSProperties = {
  flex: 1, height: '100%', border: 'none', outline: 'none', padding: '0 15px', 
  fontSize: '14px', color: '#8F8362', background: 'transparent', fontFamily: FONT_VAR
};

// จัดวางให้อยู่ขวา และให้ Tag ไหลออกมาจากทางซ้ายของปุ่ม
const tagAreaWrapper: React.CSSProperties = {
  display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: '5px', width: '100%',
  overflow: 'hidden' // กัน Tag ล้นบรรทัด
};

const tagsContainer: React.CSSProperties = {
  display: 'flex', gap: '5px', overflow: 'hidden', whiteSpace: 'nowrap',
  justifyContent: 'flex-end', alignItems: 'center'
};

// ✨ สไตล์พื้นฐานของแคปซูล (ปุ่มและ Tag ขนาดเท่ากัน)
const commonCapsule: React.CSSProperties = {
  display: 'flex',
  height: '15px', // ✨ ความสูงเท่ากันตามสั่ง
  padding: '10px 10px',
...
const tagLabelText: React.CSSProperties = { fontSize: '9px', fontFamily: FONT_VAR, color: '#8F8362' };

const removeBtnStyle: React.CSSProperties = { 
  background: 'none', border: 'none', padding: 0, cursor: 'pointer', 
  fontSize: '9px', color: '#8F8362', display: 'flex', alignItems: 'center' 
};